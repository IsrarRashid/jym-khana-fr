...
const DateRangeFilter = () => {
  ...
  const handleSelect = (ranges: RangeKeyDict) => {
    ...
    const memberType = searchParams.get("memberType") || "0"; // Default to 0
    const searchParamWord = `${
      parseInt(memberType) === 1 ? "memberType=" + memberType + "&" : ""
    }`;

    // Build the new URL with query params
    router.push(`?${searchParamWord}fromDate=${start}&toDate=${end}`);
